        ...
        <table class="table table-striped">
            <thead>
                <tr style='border:none'>
                    <th colspan="9" class='text-right'>
                        (<span id="gpnumber"></span>/1)GP = Grand Prize
                        <br>(<span id="spnumber"></span>/10)SP = Secondary Prize
                    </th>
                </tr>
                <tr>
                    <th colspan="9" class='text-right'>DQ = Disqualified</th>
                </tr>
                <tr>
                    <th>ID</th>
                    <th>Image</th>
            ...
            <tbody>
                @foreach ($candidates as $candidate)
                    <tr id='tr{{$candidate->id}}' @if ($candidate->display==true)
                        class='danger'
                    @endif>
                        <td>{{$candidate->id}}</td>
                        <td style='
                        ...
                        <td>
                            <div class="form-check">
                                <input type="checkbox" class="form-check-input sp"  id='sp{{$candidate->id}}' value="{{$candidate->id}}" @if($candidate->sp==true) checked
                                @endif
                                @if ($candidate->display==true ) disabled  @endif>
                                </label>
                            </div>
                        </td>
                        <td>
                            <div class="form-check">
                            <input type="checkbox" class="form-check-input gp" id='gp{{$candidate->id}}' value="{{$candidate->id}}" @if($candidate->gp==true)
